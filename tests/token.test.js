...
describe('signup function', () => {
  ...
  it('should call login on successful signup and return success message', async () => {
    ...
    const message = await signup('user1', 'user1@example.com', 'password123', true);
  ...
  it('should return error message on failed signup', async () => {
    ...
    const message = await signup('user1', 'user1@example.com', 'password123', false);
  ...
  it('should handle signup success but login failure correctly', async () => {
    ...
    expect(fetch).toHaveBeenCalledTimes(2);
    expect(message).toBe('Signup successful, but login failed. Please try logging in manually.');
