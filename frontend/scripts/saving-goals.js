...
const pct = (cur, tgt) => {
  const t = Number(tgt)||0, c = Number(cur)||0;
  if (!t) return 0;
...
function render(){
  ...
  // summary
  const totalTarget = data.reduce((a,g)=>a+Number(g.targetAmount||0),0);
  const totalSaved  = data.reduce((a,g)=>a+Number(g.currentSaved||0),0);
  ...
  const sorted = [...data].sort((a,b)=>{
    ...
    if (sortBy === 'progress') return pct(b.currentSaved,b.targetAmount) - pct(a.currentSaved,a.targetAmount);
  ...
  sorted.forEach(g=>{
    ...
    const badge =
      p >= 100 ? `<span class="badge good">Reached</span>` :
      dl < 0   ? `<span class="badge warn">Past due</span>` :
      dl <= 7  ? `<span class="badge soon">Due soon</span>` :
    ...
    row.querySelector('[data-action="edit"]').onclick = ()=> loadForm(g.id);
...
function saveGoal(e){
  ...
  const item = { id, name, targetAmount, currentSaved, deadline, category: selectedCategory };
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  $('#resetGoalForm').addEventListener('click', ()=>{
    $('#goalForm').reset(); $('#goalId').value=''; selectedCategory='travel'; renderCategoryPicker();
  });
