...
// Mock window.location.assign once for all tests
beforeAll(() => {
  delete window.location; // remove the real one
  window.location = { assign: jest.fn() }; // mock it
});

// Mock fetch globally for all tests
beforeEach(() => {
  global.fetch = jest.fn(); // reset before every test
  jest.clearAllMocks(); // reset mocks each run
  localStorage.clear();
