...
function render() {
    ...
    $('#nextDue').textContent = next
        ? `$${Number(next.amount || 0).toFixed(2)} • ${next.dueDate}`
        : '$0';
    ...
    `;
        ...
        row.querySelector('[data-action="toggle"]').onclick = () => {
            ...
            if (i >= 0) {
                arr[i].status = arr[i].status === 'paid' ? 'pending' : 'paid';
                write(arr);
                render();
            }
        };
...
function save(e) {
    ...
    if (!description || !(amount > 0))
        return alert('Enter a description and positive amount.');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $('#resetPaymentForm').addEventListener('click', () => {
        $('#paymentForm').reset();
        $('#paymentId').value = '';
        selectedCategory = 'bills';
        renderCategoryPicker();
