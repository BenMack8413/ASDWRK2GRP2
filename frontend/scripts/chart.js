...
function renderChart(config) {
    ...
    const startDate = configData.startDate
        ? new Date(configData.startDate).toLocaleDateString()
        : 'N/A';
    const endDate = configData.endDate
        ? new Date(configData.endDate).toLocaleDateString()
        : 'N/A';
...
async function addChart() {
    ...
    try {
        ...
        if (dataSource === 'expenses-by-category') {
            chartData = await fetchExpensesByCategory(
                budgetId,
                startDate,
                endDate,
            );
...
async function deleteChart(configId, btn) {
    ...
    try {
        const res = await fetch(`/api/charts/configs/${configId}`, {
            method: 'DELETE',
        });
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Hook up buttons
    document
        .getElementById('addChartBtn')
        .addEventListener('click', () => toggleChartForm(true));
    document
        .getElementById('cancelChartBtn')
        .addEventListener('click', () => toggleChartForm(false));
