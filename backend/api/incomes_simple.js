...
module.exports = function createSimpleIncomeRouter() {
  ...
  router.get('/', (req, res) => {
    ...
    const rows = sharedDb
      .prepare('SELECT * FROM incomes WHERE budget_id = ? ORDER BY date DESC, income_id DESC')
      .all(budget_id);
  ...
  router.post('/', (req, res) => {
    try {
      ...
      const budget = sharedDb.prepare('SELECT budget_id FROM budgets WHERE budget_id = ?').get(Number(budget_id));
      if (!budget) return res.status(400).json({ error: 'budget_not_found', detail: `Budget ${budget_id} not found` });

      // Validate optional account if provided
      if (account_id != null) {
        const acc = sharedDb.prepare('SELECT account_id FROM accounts WHERE account_id = ? AND budget_id = ?').get(Number(account_id), Number(budget_id));
        if (!acc) {
          return res.status(400).json({ error: 'account_not_found_or_mismatch', detail: `Account ${account_id} not found for budget ${budget_id}` });
        }
      }
      ...
      const info = sharedDb.prepare(
        `INSERT INTO incomes (budget_id, user_id, account_id, amount, source, date, frequency, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(Number(budget_id), user_id || null, account_id || null, amount_cents, source || null, String(date), frequency || null, notes || null);
