...
module.exports = function createTransactionsRouter(db, createTransactionAtomic) {
  ...
  router.get('/', (req, res) => {
    ...
    const rows = db
      .prepare('SELECT * FROM transactions WHERE budget_id = ? ORDER BY date DESC, transaction_id DESC')
      .all(budget_id);
  ...
  router.get('/:id', (req, res) => {
    ...
    if (!transaction_id) return res.status(400).json({ error: 'transaction_id query parameter is required' });
    const budget_id = Number(req.query.budget_id);
    if (!budget_id) return res.status(400).json({ error: 'budget_id query parameter is required' });
  ...
  router.post('/', (req, res) => {
    try {
      ...
      if (!payload || !payload.budget_id || !Array.isArray(payload.lines) || payload.lines.length === 0) {
        return res.status(400).json({ error: 'budget_id and at least one line required' });
      }
      ...
      const account = txn.account_id
        ? db
            .prepare('SELECT account_id, balance FROM accounts WHERE account_id = ? AND budget_id = ?')
